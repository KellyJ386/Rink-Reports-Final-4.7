...
import { submitAirQuality } from './actions'
...
export function NewAirQualityClient({ sections }: Props) {
  ...
  return (
    <DynamicForm
      ...
      onSubmit={async (values: DynamicFormValues): Promise<DynamicFormSubmitResult> => {
        const result = await submitAirQuality(values, idempotencyKey)
        if (result.ok) {
          router.push('/modules/air-quality')
          router.refresh()
        }
        return result
      }}
