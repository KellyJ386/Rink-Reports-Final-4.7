...
import { submitRefrigeration } from './actions'
...
export function NewRefrigerationClient({ sections }: Props) {
  ...
  return (
    <DynamicForm
      sections={sections}
      submitLabel="File reading"
      onSubmit={async (values: DynamicFormValues): Promise<DynamicFormSubmitResult> => {
        const result = await submitRefrigeration(values, idempotencyKey)
        if (result.ok) {
          router.push('/modules/refrigeration')
          router.refresh()
        }
        return result
      }}
