...
import { NewIncidentClient } from './client'

export default async function NewIncidentPage() {
  await requireModuleEnabled('incident')
  const loaded = await loadPublishedFormSchema('incident', null)
  if (!loaded) {
    return (
      <main>
        <h1 className="text-xl font-semibold">Incident reports unavailable</h1>
  ...
  if (loaded.renderErrors.length > 0) {
    return (
      <main>
        <h1 className="text-xl font-semibold">New incident report</h1>
        <div className="mt-4">
  ...
  return (
    <main>
      ...
      <div className="mt-6">
        <NewIncidentClient sections={loaded.schema.sections} />
      </div>
    </main>
  )
}
